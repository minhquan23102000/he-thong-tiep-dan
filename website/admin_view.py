...
class MyAdminIndexView(AdminIndexView):
    ...
    def index(self):
        if (
            ...
            and session.get("user_role", None) == Role.ADMIN.value
        ):
...
class RelearnView(MyModelView):
    ...
    def edit_view(self):
        ...
        children = (
            db.session.query(Statement).join(Statement.tags).filter(Tag.id == tag)
        )

        self._template_args.update(
            dict(children=children, edit_url=url_for("statement.edit_view", id=id))
        )
