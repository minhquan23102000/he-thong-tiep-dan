var _bsap = {
    ...
    deploy: function() {
        for (var zi = 0, wi, z, a, b, db; zi < this.jz.length && (z = this.jz[zi]) && (wi = 1); zi++)
            while (++wi < 10 && (db = this.dz[z.id]) && (a = db.pop()) && (b = document.getElementById(a)))(this.inv[z.id] = function() {
    ...
    zone: function(b, z, bsa) {
        ...
        function flushWrite(which) {
            if (bsa.writes[which]) {
                var el = document.getElementById(which),
                    writer = el.parentNode.insertBefore(document.createElement('div'), el);
            ...
            bsa.scriptqueue.push({
                ...
                run: (function(id, sc) {
                    return function() {
                        ...
                        if (sc.src) bsa.writescript.src = sc.src;
                        else {
                            try {
        ...
        bsa.filter(z.filterby, function(f) {
            ...
            zf = !zf.length ? (zs.all || z) : {
                ads: zf
            };
            var empty = 0,
                exec = [],
                c = (!zf || !zf.ads || !zf.ads.length) ? [] : bsa.getads(zf, z.nads),
                d, e = c[0],
                t = '',
                o = '',
                fr, a, backfilled = false,
                tf = z.format ? z.format : (z.type && z.type == 1 ? 2 : 0),
            ...
            if (zf && zf.ads)
                for (empty = 100, i = 0; i < zf.ads.length; i++) empty -= zf.ads[i].per / (z.model == 1 ? z.nads : 1);
            ...
            if (tf == 0) {
                ...
                for (i = j;
                    (typeof(bsa.callback) == 'function') && i < z.nads; i++, o += bsa.callback(z.id, i - 1, z.nads) || '') {}
                o += '<\/ul>';
            ...
            if (o && !b.innerHTML.length) {
                b.innerHTML = o;
                if (exec.length)
                    for (i = 0; i < exec.length; i++) setInnerHtmlAndExec(document.getElementById(exec[i][0]), exec[i][1])
            }
    ...
    filter: function(fs, fn, noto) {
        ...
        if (fs.geo) {
            ...
            if (i >= 0) IAm(c.substring(i + 7 + 1).split(';')[0]);
            else {
    ...
    rocks: function(t, b, z) {
        var u = this.gen('bsau', 30),
            s = this.gen('bsas', 1 / 48),
            img = new Image();
    ...
    gen: function(w, e) {
        var c = document.cookie,
            i = c.indexOf(w + '=');
        if (i >= 0) return c.substring(i + w.length + 1).split(';')[0];
        else {
            var d = new Date(),
                nd = +d;
