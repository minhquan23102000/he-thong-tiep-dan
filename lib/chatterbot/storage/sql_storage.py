...
from lib.chatterbot.storage.storage_adapter import StorageAdapter
...
class SQLStorageAdapter(StorageAdapter):
    ...
    def model_to_object(self, statement):
        from lib.chatterbot.conversation import Statement as StatementObject
    ...
    def filter(self, **kwargs):
        ...
        if exclude_text:
            statements = statements.filter(
                ~Statement.in_response_to.in_(exclude_text)
            )

        if exclude_text_words:
            or_word_query = [
                Statement.in_response_to.ilike('%' + word + '%') for word in exclude_text_words
            ]
            statements = statements.filter(
                ~or_(*or_word_query)
            )
    ...
    def create(self, **kwargs):
        ...
        if 'search_text' not in kwargs:
            kwargs['search_text'] = self.tagger.get_bigram_pair_string(
                kwargs['text'])

        if 'search_in_response_to' not in kwargs:
            in_response_to = kwargs.get('in_response_to')
            if in_response_to:
                kwargs['search_in_response_to'] = self.tagger.get_bigram_pair_string(
                    in_response_to)
    ...
    def create_many(self, statements):
        ...
        for statement in statements:
            ...
            if tag_data:
                ...
                if existing_tags:
                    create_tags[existing_tags.name] = existing_tags

            if tag_data in create_tags:
                tag = create_tags[tag_data]
            else:
                # Create the tag if it does not exist
                tag = Tag(name=tag_data)

                create_tags[tag_data] = tag
    ...
    def update(self, statement):
        ...
        if statement is not None:
            ...
            record.search_text = self.tagger.get_bigram_pair_string(
                statement.text)
